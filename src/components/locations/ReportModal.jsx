...
export default function ReportModal({ locationId, locationName, onClose }) {
  ...
  const handleSubmit = async () => {
    if (!reason) return;
    // Fix #6: cap details length
    if (details.length > 500) {
      alert("Details must be under 500 characters.");
      return;
    }
    setSubmitting(true);
    try {
      const { error } = await supabase
        .from("reports")
        .insert([{ location_id: locationId, reason, details: details.slice(0, 500) }]);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", backdropFilter: "blur(4px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#0d0d0d", border: "1px solid #2a2a2a", width: 400, maxWidth: "95vw", padding: 24 }}>
        {done ? (
          <>
            ...
            <button onClick={onClose} style={{ background: "#c8b89a", color: "#0a0a0a", border: "none", padding: "10px 20px", fontFamily: "'Bebas Neue'", fontSize: 14, letterSpacing: 2, cursor: "pointer" }}>CLOSE</button>
          </>
        ) : (
          <>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <div style={{ fontSize: 9, color: "#666", letterSpacing: 2 }}>DETAILS (OPTIONAL)</div>
                <div style={{ fontSize: 9, color: details.length > 450 ? "#f87171" : "#555" }}>{details.length}/500</div>
              </div>
              <textarea
                value={details}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={onClose} style={{ flex: 1, background: "transparent", border: "1px solid #2a2a2a", color: "#666", padding: "10px", fontFamily: "'Bebas Neue'", fontSize: 13, letterSpacing: 2, cursor: "pointer" }}>CANCEL</button>
              <button onClick={handleSubmit} disabled={!reason || submitting} style={{ flex: 2, background: "#f87171", color: "#0a0a0a", border: "none", padding: "10px", fontFamily: "'Bebas Neue'", fontSize: 13, letterSpacing: 2, cursor: (!reason || submitting) ? "not-allowed" : "pointer", opacity: (!reason || submitting) ? 0.4 : 1 }}>
